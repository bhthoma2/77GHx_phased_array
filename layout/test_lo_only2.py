...
def main():
    ...
    # RF_N (M4 in channel, M5 stubs into blocks)
    lna_r_edge = lna_x + 300.0
    mix_l_edge = mix_x
    lna_n_route_y = lna_port_y - 5.0
    mix_n_rf_y = mix_rf_y - 5.0
    ...
    # INJ_P (M5, branches off LO_P channel)
    ...
    # INJ_N DISABLED FOR DEBUG

    # TX_N (M4 in channel, M5 stub into TXPA)
    txpa_r_edge = txpa_x + 350.0
    txpa_n_entry_y = txpa_port_y - 10.0
    tx_outn_h_y = vco_port_y - 10.0
    top.shapes(ly['M4']).insert(pya.Box(
        um(vco_x + 250.0 - route_w/2), um(tx_outn_h_y - route_w/2),
    ...
    top.shapes(ly['M5']).insert(pya.Box(
        ...
        um(txpa_r_edge + route_w/2), um(txpa_n_entry_y + route_w/2)))
    top.shapes(ly['M5']).insert(pya.Box(
        um(txpa_inn_x - route_w/2), um(txpa_port_y - route_w/2),
        um(txpa_inn_x + route_w/2), um(txpa_n_entry_y + route_w/2)))

    # LO_N (M4 in channel, M5 stub into Mixer)
    outn_h_y = vco_port_y - 10.0
    mix_r_edge = mix_x + 300.0
    top.shapes(ly['M4']).insert(pya.Box(
    ...
    # --- OUTPUT ---
    output = OUT_DIR + "test_lo_only2.gds"
    layout.write(output)
